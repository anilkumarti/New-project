...
if (!process.env.GEMINI_API_KEY) {
  ...
  if (envResult.parsed) {
    console.log(`Loaded fallback env from ${rootEnvPath}`);
  }
}
if (!process.env.GEMINI_API_KEY) {
  console.warn('GEMINI_API_KEY not found in server/.env or ../.env. Make sure the proxy server is started from fat-kill/server and that server/.env contains the key.');
}
...
// --- Validation helpers ---
// These helpers enforce that only fitness-related queries are allowed.
// Validation is applied both on the frontend and backend to prevent
// unintended or malicious prompts reaching the Gemini API.
// Note: free Gemini models (when accessed directly) may attempt to
// follow user instructions that override system prompts, so we block
// disallowed queries server-side as a final safeguard.
...
// Basic sanitization to reduce prompt-injection surface area
const sanitizePrompt = (text) => {
  if (!text || typeof text !== 'string') return '';
  let p = text.replace(/```[\s\S]*?```/g, ''); // remove code fences
  p = p.replace(/<[^>]+>/g, ''); // remove HTML tags
  // neutralize common prompt injection phrases
  PROMPT_INJECTION_PATTERNS.forEach((rx) => {
    p = p.replace(rx, '[redacted]');
  });
  // collapse whitespace and truncate to reasonable length
  p = p.replace(/\s+/g, ' ').trim().slice(0, 2000);
  return p;
};

// Returns true only when prompt appears to be about allowed fitness topics
const isFitnessQuery = (text) => {
  if (!text || typeof text !== 'string') return false;
  const lower = text.toLowerCase();

  // If it contains any clearly blocked category, reject immediately
  for (const rx of BLOCKED_CATEGORY_PATTERNS) {
    if (rx.test(lower)) return false;
  }

  // Must contain at least one allowed keyword to be considered valid
  for (const kw of ALLOWED_KEYWORDS) {
    const rx = new RegExp('\\b' + kw.replace(/[-\\/\\^$*+?.()|[\]{}]/g, '\\$&') + '\\b', 'i');
    if (rx.test(lower)) return true;
  }

  // As a fallback, allow short direct questions that mention calories/food/workout
  if (/\b(calorie|calories|workout|exercise|diet|nutrition|protein|gym|yoga|cardio|bmi)\b/i.test(lower)) return true;

  return false;
};

// A strict system prompt prepended to user prompts sent to Gemini.
// This is an important safety layer, but note that free or misconfigured models
// might attempt to obey user-supplied directives that conflict with system prompts.
// Server-side validation above ensures disallowed queries are blocked regardless.
...
app.post('/api/gemini', async (req, res) => {
  try {
    ...
    if (!models.length && !preferredModel) {
      return res.status(502).json({ error: 'No Gemini models are available for this API key.' });
    }

    // Sanitize and validate the incoming prompt to enforce scope restrictions.
    const sanitizedPrompt = sanitizePrompt(prompt);
    if (!isFitnessQuery(sanitizedPrompt)) {
      // Return the exact restricted response when the query is outside allowed topics.
      return res.json({ reply: getRestrictedResponse() });
    }

    // Prepend a strict system prompt to instruct Gemini about the role/rules.
    // Note: This is supplemental to server-side validation — system prompts can
    // be ignored by some model behaviors, so the server-side block above is necessary.
    const finalPrompt = `${SYSTEM_PROMPT}\n\nUser Prompt: ${sanitizedPrompt}`;

    const primary = await callGemini(apiKey, model, finalPrompt);
    if (primary.ok) {
      return res.json({ reply: primary.reply || JSON.stringify(primary.json || {}), raw: primary.json, model, endpoint: primary.endpoint });
    }

    console.error('Gemini primary failure', { model, attempts: primary.attempts.map((a) => ({ endpoint: a.endpoint, status: a.status, text: a.text })) });
    ...
    for (const fallbackModel of fallbackCandidates) {
      ...
      if (retry.ok) {
        return res.json({ reply: retry.reply || JSON.stringify(retry.json || {}), raw: retry.json, model: fallbackModel, endpoint: retry.endpoint });
      }
      console.error('Gemini fallback failure', { fallbackModel, attempts: retry.attempts.map((a) => ({ endpoint: a.endpoint, status: a.status, text: a.text })) });
    }
